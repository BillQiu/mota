...
- **24 层**：经典数据 1–9 层（完整连通段）+ 自建续塔 10–23 层 + 魔王殿（24）
- **精灵图**（`public/sprites/`）也取自该项目：RPG Maker 风格的勇士/怪物/钥匙/门/宝石/楼梯，观感对齐原版
- 忠实复刻该版本机制：**勇士初始 400 生命 / 攻 10 / 防 10**；**宝石、血瓶按楼层缩放**（红宝石 +楼层 攻、红瓶回 50×楼层…）；四色钥匙
- 转换脚本见 `scripts/convert-justinmzt.mjs`，生成 `src/data/imported.ts`

## 一键自动

工具栏「自动」按钮调用内置**跨层贪心求解器**（`src/core/solver.ts`）自动通关：全塔跨楼梯 BFS 找最优目标、钥匙规划（保守开门 + 商店囤蓝/红钥匙）、保命战斗。该求解器同时作为「全 24 层可通关」的回归测试。
